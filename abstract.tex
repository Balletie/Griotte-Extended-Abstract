...
\todo{requires expanding}requesting a review for a change requires too much effort from
  the perspective of the developer.
...
performed to get to that state are lost, and collapsed into one
change. These intermediate actions are known as \textit{fine-grained
  IDE events} --- in contrast with the \textit{coarse-grained} changes
one might find in a VCS commit.
...
is to group changes. \todo{How does this solve these problems?}

Tao and Kim\cite{Tao15a} have explored grouping (referred to as
\textit{partitioning}) tangled commits (\textit{composite changes}) in
the context of code review. We aim to expand on this solution by
grouping changes using fine-grained IDE events instead of the more
coarse-grained information available in a VCS repository.
